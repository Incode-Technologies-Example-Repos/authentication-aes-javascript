...
const adminHeader = {
  'Content-Type': 'application/json',
  'api-version': '1.0',
  'X-Incode-Hardware-Id': import.meta.env.VITE_FAKE_BACKEND_ADMINTOKEN
}

// Call Incode's `omni/start` API to create an Incode session which will include a
// token in the response.
const fakeBackendAuth = async function (token, transactionId, interviewToken) {
  const url = `${apiurl}/omni/authentication/verify`
  const params = { token, transactionId, interviewToken }

  let response
  try {
    response = await fetch(url, { method: 'POST', body: JSON.stringify(params), headers: adminHeader })
    if (!response.ok) {
...
//   const sessionHeaders = { ...defaultHeader }
